...
public class Scaffold extends Module {
    ...
    //Credits to Fan Client cuz I'm lazy to make scaffold lol

    public static float yaw;
    public static float pitch;
    private final Stopwatch timerTower = new Stopwatch();
    ...
    public BoolSetting
            boost = new BoolSetting("Boost", false);//,
    public NumberSetting delay = new NumberSetting("Place Delay", 50, 0, 500,10, true),
            boostDuration = new NumberSetting("Boost Duration", 1500, 0, 3000,50, true),
    //speed = new NumberSetting("Move Speed", 0.05, 0, 0.2, 0.01),
    placeRandomization = new NumberSetting("Randomization", 50, 0, 100,10, true);
    public static BoolSetting toggleFly = new BoolSetting("Toggle Fly", true),
            sprint = new BoolSetting("Sprint", true),
            tower = new BoolSetting("Tower", true);
    ...
    int currentItem;
    double groundy = 0;
    Stopwatch timerMotion = new Stopwatch(), boostCap = new Stopwatch();
    private int slot;

    public Scaffold() {
        super("Scaffold", "hgfhfg", Keyboard.KEY_C, Category.MOVEMENT);
        addSettings(delay, placeRandomization, tower, toggleFly);
        setScaffoldValues();
    }
    ...
    public void onEnable() {
        ...
        if (toggleFly.isEnable())
            Client.moduleManager.getModuleByName("Flight").setEnabled(false);
        if(!sprint.isEnable()){
    ...
    public void onEvent(Event ev) {
        if (ev instanceof EventUpdate) {
            this.setDisplayName("Scaffold " + ChatFormatting.GRAY + "Normal");
            if (mc.thePlayer.swingProgress <= 0.0f && !mc.thePlayer.isEating() && !Client.moduleManager.getModuleByName("KillAura").isEnabled()) {
        ...
        if (e.isPre()) {
            ...
            if ((mc.gameSettings.keyBindJump.isKeyDown()) && (mc.thePlayer.moveForward == 0.0F) && (mc.thePlayer.moveStrafing == 0.0F) && tower.isEnable()) {
        ...
        if ((localBlockData != null)) {
            if (e.isPre()) {
                ...
                mc.thePlayer.rotationYawHead = arrayOfFloat[0];
                mc.thePlayer.renderYawOffset = arrayOfFloat[0];
                mc.thePlayer.rotationPitchHead = arrayOfFloat[1];
                ...
                if (!this.timerMotion.elapsed((long) (delay.getValue() + Math.random() * 4))
    ...
    public float[] getRotationsHypixel(BlockPos paramBlockPos, EnumFacing paramEnumFacing) {
        double d1 = paramBlockPos.getX() + 0.5D - mc.thePlayer.posX + paramEnumFacing.getFrontOffsetX();
        double d2 = paramBlockPos.getZ() + 0.5D - mc.thePlayer.posZ + paramEnumFacing.getFrontOffsetZ();
        double d3 = mc.thePlayer.posY + mc.thePlayer.getEyeHeight() - (paramBlockPos.getY());
        double d4 = MathHelper.sqrt_double(d1 * d1 + d2 * d2);
        float f1 = (float) (Math.atan2(d2, d1) * 180 / Math.PI);
